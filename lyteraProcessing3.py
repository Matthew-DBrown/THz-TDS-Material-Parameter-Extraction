...
def plot_epsilonR_summary(file_paths, xlims=None, label_cond='side', color_set=colors_13, radix_inset=False, title=None):
    ...
    for path in file_paths:
        ...
        for file, file_obj in sample_files.items():
            ...
            epsilonR_data_tot_list.append(epsilon_i/epsilon_r)
            #print(min_idx, " ", max_idx)
        epsilonR_data_stack = np.stack(epsilonR_data_tot_list, axis=0)
        ...
        ax.plot(f_data, smoothed_avg, linestyle='-', linewidth=2, color=color_set[i], label=f"{temp_string}")
        ax.set_xlim(np.min(f_data), np.max(f_data))
        ax.set_ylabel("$\epsilon_r$", fontsize=14)
        ax.set_xlabel("Frequency (THz)", fontsize=14)
        #plt.plot(f_data, np.average(n_data_stack, axis=0), color='r', linewidth=2, label='Average')
...
def plot_lossTan_summary(file_paths, xlims=None, label_cond='side', color_set=colors_13, radix_inset=False, title=None):
    ...
    for path in file_paths:
        ...
        ax.plot(f_data, smoothed_avg, linestyle='-', linewidth=2, color=color_set[i], label=f"{temp_string}")
        ax.set_xlim(np.min(f_data), np.max(f_data))
        ax.set_ylabel("Loss Tangent, $\tan(\delta)$", fontsize=14)
        ax.set_xlabel("Frequency (THz)", fontsize=14)
        #plt.plot(f_data, np.average(n_data_stack, axis=0), color='r', linewidth=2, label='Average')
...
radix_46_film_path = r"C:\Users\matth\OneDrive - University of Birmingham\Y4 Project\TERA_BACKUP\y4_project\radix_4.6\lyera_focused_radix4.6_film"
...
#plot_avg_ref_ind(tmm3_files)
#plot_avg_k(tmm3_files)
#plot_ref_ind(tmm3_files)
#plot_k(tmm3_files)

#plot_avg_ref_ind(radix_28_6_files, xlims=[0.3, 1], title='Radix 2.8(6)')
#plot_n_summary(paths_list, xlims=[0.3, 1.4], label_cond='legend', color_set=colors_13, title=None)
#plot_n_summary(paths_list_set1, xlims=[0.3, 1.4], color_set=sub_colors2, title=None)
#plot_n_summary(paths_list_set2, xlims=[0.3, 1.4], label_cond='legend', color_set=sub_colors3, radix_inset=True, title=None)
plot_n_summary(paths_list, xlims=[0.3, 1.4], label_cond='ontop', color_set=colors_13, radix_inset=True, title=None)
plot_epsilonR_summary(paths_list, xlims=[0.3, 1.4], label_cond='legend-outside', color_set=colors_13, radix_inset=True, title=None)
plot_lossTan_summary(paths_list_set1, xlims=[0.3, 1.4], label_cond='ontop', color_set=sub_colors3, radix_inset=False, title=None)
